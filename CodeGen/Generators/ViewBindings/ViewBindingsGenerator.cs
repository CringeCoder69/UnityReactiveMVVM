...
namespace CodeGen.Generators.ViewBindings
{
    ...
    internal partial class ViewBindingsGenerator : ISourceGenerator
    {
        ...
        private string Build(GenerationData data)
        {
            ...
            sb.AppendLine("        }");
            sb.AppendLine($"        protected override void BindSelf({data.InterfaceName} viewModel)");
            sb.AppendLine("        {");
            sb.AppendLine("            base.BindSelf(viewModel);");

            foreach (var button in data.ReactiveCommands)
                sb.AppendLine($"            Bind({button.Name}.Clicked, viewModel.{button.Name}.Execute);");

            foreach (var observable in data.Observables)
                sb.AppendLine($"            Bind(viewModel.{observable.Name}, {observable.Name}.Invoke);");
